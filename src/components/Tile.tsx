...
export default function Tile({ title, link, description, tools = [], image, size = "cover", position = "0 0", expanded = false, addStyle, onClick }: Props) {
    const handleClick = () => {
        onClick?.();
    }
    ...
    return (
        <article
            ...
            className={
                `group relative w-96 h-96 p-4 border-2 hover:border-teal-500 rounded-2xl overflow-hidden cursor-pointer bg-no-repeat transition-all duration-500
        ...
        >
            ...
            >
                <h3 className="text-xl font-semibold mb-1 leading-loose">
                    <a href={link} target="_blank" tabIndex={expanded ? 0 : -1} onClick={handleLinkClick}>{title} <ExternalLink className="inline-block align-middle w-6 h-6" /></a>
                </h3>
                <Markdown
                    components={{
                        li: ({ ...props }) => <li className="ml-4 my-4 pl-1 text-xl list-disc mb-1" {...props} />
                    }}
                >
                    {description}
                </Markdown>
